#include "Person.h" // подключаем .h файл нашего челика



Person::Person() {
	texture.loadFromFile("images/person.png"); //загружаем текстуру
...
void Person::animation(float time) {
	...
	*/
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) {
		CurrentFrame += 0.005 * time;
		if (CurrentFrame > 4) CurrentFrame -= 4;
		sprite.setTextureRect(sf::IntRect(225 * int(CurrentFrame), 1525, 225, 355));
		sprite.setPosition(sprite.getPosition().x - 0.2, sprite.getPosition().y);
	}
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::A)) {
		CurrentFrame += 0.005 * time;
		if (CurrentFrame > 4) CurrentFrame -= 4;
		sprite.setTextureRect(sf::IntRect(225 * int(CurrentFrame), 1886, 225, 386));
		sprite.setPosition(sprite.getPosition().x - 0.5, sprite.getPosition().y);
	}
